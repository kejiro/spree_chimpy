...
describe Spree::Chimpy do
  context "enabled" do
    before do
      allow(Spree::Chimpy::Interface::List).to receive(:new) {:list}
      allow(Spree::Chimpy::Interface::Orders).to receive(:new) {:orders}
  ...
  context "sync merge vars" do
    ...
    before do
      allow(Spree::Chimpy::Interface::List).to receive(:new) {interface}
      config(key: '1234',
             list_name: 'Members',
